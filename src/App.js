...
function App() {
  ...
  useEffect(() => {
    ...
    if(gender === 'boy' || gender === 'girl' || gender === 'neutral') {
      ...
      .catch(error => {
        console.error('A problem occured trying to fetch from api-Ninjas:', error);
        setSearchError(true);
      });
    }
  }, [gender, apiKey]); // the dependency array specifies which values/variables should be watched for changes
